...
describe("LOSING_SIDE_PRESSURE", () => {
  ...
  it("scales monotonically with losing-side hostility", () => {
    ...
    const severe = losingSidePressure(
      report({ threats: [threatDigit(0, 90), threatDigit(1, 80)], risingLosers: [0, 1], groupThreat: 85 }),
    );
  ...
  it("dampens an opportunity and records the cost", () => {
    const p = losingSidePressure(report({ threats: [threatDigit(0, 90)], risingLosers: [0], groupThreat: 80 }));
