...
function progressFor(p: OperatorPattern): OperatorPatternProgress {
  ...
  const tradesRemaining =
    requiredForNextStage === null
      ? null
      : Math.max(0, requiredForNextStage - subsequentTrades);
...
export function summariseOperatorLearning(
  ...
  const counts: OperatorLearningCounts = {
    ...
    beingTested: patterns.filter(
      (p) => p.status === "OBSERVATION" || p.status === "EMERGING",
    ).length,
  ...
  const entry =
    Math.round(patterns.reduce((a, p) => a + p.entryAdjustment, 0) * 10) / 10;
  const ranking =
    Math.round(patterns.reduce((a, p) => a + p.rankingAdjustment, 0) * 10) / 10;
