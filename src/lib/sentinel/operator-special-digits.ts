...
export function operatorSpecialDigitAction(
  ...
  if (p.frequencyVelocity > 0.01) {
    ...
    drivers.push(`recent frequency rising (${(p.fast * 100).toFixed(1)}% fast vs ${(p.medium * 100).toFixed(1)}% medium)`);
  }
  ...
  if (p.transitionInflow >= 0.14) {
    ...
    drivers.push(`transition inflow ${(p.transitionInflow * 100).toFixed(0)}% from the current digit`);
  }
  if (p.anomaly >= 55) {
