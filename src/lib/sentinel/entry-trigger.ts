...
export function classifyTouches(input: EntryTriggerInputs): {
  ...
  const { prints, absenceThreshold, meanGap, sinceSeen } = classifyPrints(
    digits,
    input.entryDigit,
  );
...
export function computeEntryTrigger(input: EntryTriggerInputs): EntryTriggerReport {
  ...
  const firstUpper = first.n ? Math.min(1, first.immediateWinRate + (first.immediateWinRate - first.immediateLower)) : 1;
  const subUpper = subsequent.n
    ? Math.min(1, subsequent.immediateWinRate + (subsequent.immediateWinRate - subsequent.immediateLower))
    : 1;
  const separationSignificant =
    bothSufficient &&
    (first.immediateLower > subUpper || subsequent.immediateLower > firstUpper);
  ...
  if (preferredTouch) {
    ...
    rankingDelta =
      Math.round(
        clamp(edgePart + alignPart + stabilityPart, -MAX_ENTRY_TRIGGER_DELTA, MAX_ENTRY_TRIGGER_DELTA) *
          10,
      ) / 10;
  }
  ...
  if (!lines.length)
    lines.push(`No classified print of digit ${input.entryDigit} has a following run in the causal buffer.`);
