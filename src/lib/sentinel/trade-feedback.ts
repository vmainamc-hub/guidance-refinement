...
function load(): Store {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && (parsed.version === 1 || parsed.version === 2) && Array.isArray(parsed.trades)) {
        store = {
          version: 2,
...
export function mergeRemoteFeedback(
  ...
  for (const rt of remoteTrades) {
    ...
    const localRev = Math.max(local.resolvedAt ?? 0, local.feedback?.updatedAt ?? local.feedback?.ts ?? 0, local.ts);
    const remoteRev = Math.max(rt.resolvedAt ?? 0, rt.feedback?.updatedAt ?? rt.feedback?.ts ?? 0, rt.ts);
...
export function observationCategoryCounts(
  symbol?: string,
  contract?: string,
): CategoryCount[] {
...
export function allLearning(): MarketLearning[] {
  const pairs = new Set(confirmedTrades().map((t) => `${t.snapshot.symbol}|${t.snapshot.contract}`));
  return [...pairs]
    .map((p) => {
      const [sym, con] = p.split("|");
