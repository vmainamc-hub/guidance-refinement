...
import { isVetoedRoleDigit, VETOED_ROLE_DIGITS, assignDigitRoles } from "../digit-roles";
...
describe("immediate operator guidance (Channel 1)", () => {
  ...
  it("interprets an explicit late-entry note as an entry-timing directive", () => {
    const d = interpretFeedback("I entered too late", "ENTRY TOO LATE", { entryDigit: 4 });
  ...
  it("never turns ambiguous text into a directional rule", () => {
    const d = interpretFeedback("hmm", null, { entryDigit: 4 });
  ...
  it("decays over its lifetime and expires completely", () => {
    ...
    expect(immediateGuidanceLookup(now + DEFAULT_TTL_MS + 1).forCandidate("R_10", "under7").active)
      .toBe(false);
  });
...
describe("legacy digit veto is retired", () => {
  it("exposes no vetoed digits and never vetoes", () => {
    expect(VETOED_ROLE_DIGITS).toHaveLength(0);
    for (let d = 0; d < 10; d++) expect(isVetoedRoleDigit(d)).toBe(false);
  });
  ...
  it("lets a former barrier digit hold a colour role", () => {
    ...
    expect(roles.vetoApplied).toBe(false);
  });
});
