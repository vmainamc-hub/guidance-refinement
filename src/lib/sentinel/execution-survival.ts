 ...
 */
export function buildExecutionSequences(
  input: ExecutionSurvivalInputs,
): { sequences: ExecutionSequence[]; windowRuns: number } {
 ...
 */
export function computeExecutionSurvival(
  input: ExecutionSurvivalInputs,
): ExecutionSurvivalReport {
  const cfg = { ...DEFAULT_EXECUTION_CONFIG, ...(input.config ?? {}) };
...
export function classifySignalPackage(input: {
  ...
  if (!input.marketQualifies)
    return {
      level: "NOT QUALIFIED",
      explanation: "The market/contract edge itself does not qualify — entry and survival are not considered.",
    };
