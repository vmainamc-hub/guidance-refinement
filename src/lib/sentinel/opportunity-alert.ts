...
export function qualify(o: RankedOpportunity, cfg: AlertConfig): Qualification {
  ...
  const survFragile = survSufficient && (surv!.label === "FRAGILE" || surv!.lossOnFirstRunRate > 0.5);
  ...
  if (!survSufficient)
    cautions.push("Post-entry multi-run behaviour is not yet measured here; treat the horizon as unproven.");
  else if (surv!.deteriorationPoint)
    cautions.push(`Measured deterioration from run ${surv!.deteriorationPoint} — do not extend the sequence past it.`);
  ...
  if (trig && trig.verdict === "INSUFFICIENT TRIGGER HISTORY")
    cautions.push("First-versus-subsequent touch behaviour is not yet measured on this entry digit.");
