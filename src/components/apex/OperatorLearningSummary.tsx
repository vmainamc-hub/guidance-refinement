  ...
  return (
    <div className="mt-3 rounded border border-border/60 p-2">
      ...
      <p className="mt-1 text-[10px] text-muted-foreground">
        Remembered is not validated, and validated never means unbounded control — influence
        stays capped and market/contract/entry-digit isolated.
      </p>
    </div>
  );
}
          ...
          <ul className="mt-1 space-y-1.5 text-[11px]">
            {inProgress.slice(0, 6).map((p) => (
              <li key={p.pattern.key} className="rounded border border-border/60 p-2">
                ...
                {p.pattern.samples?.[0] ? (
                  <p className="mt-0.5 text-muted-foreground">&ldquo;{p.pattern.samples[0]}&rdquo;</p>
                ) : null}
