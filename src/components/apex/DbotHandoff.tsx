...
export default function DbotHandoff({ item }: { item: RankedOpportunity }) {
  ...
  const survivalTone = !surv || !surv.sufficient
    ? "warn"
    : surv.label === "FRAGILE"
      ? "bear"
      : surv.label === "STRONG"
  ...
  const triggerTone = !trig || !trig.preferredTouch
    ? "warn"
    : signal?.trigger.armed
      ? "bull"
      : "bear";
      ...
      {trig && d && !waitForEntry ? (
        ...
        >
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            Measured trigger evidence only — the authoritative instruction above already resolves
            it{skipNext ? " (the next print is not the favoured cohort)" : ""}.
          </p>
          <ul className="mt-2 space-y-1">
            {trig.lines.map((line) => (
              <li key={line} className="font-mono text-[10px] leading-relaxed text-muted-foreground">
                {line}
              </li>
            ))}
          </ul>
