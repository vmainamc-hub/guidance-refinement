...
export default function DigitPsychologyPanel({ item }: { item: RankedOpportunity }) {
  ...
  const verdictTone =
    p.verdict === "SUPPORT" ? "var(--bull)" : p.verdict === "CONFLICT" ? "var(--bear)" : "var(--warn)";
  ...
  return (
    <section className="mt-5 rounded-lg border border-border/50 bg-background/40 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: verdictTone }}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        {state.map((r) => (
          <Chip
            ...
            tone={
              r.support > 0 ? "var(--bull)" : r.support < 0 ? "var(--bear)" : undefined
            }
          />
        ))}
      </div>
