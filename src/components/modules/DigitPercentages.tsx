...
export function DigitPercentages({ ticks }: { ticks: Tick[] }) {
  const stats = useMemo(() => {
    ...
    const { freq, pct } = digitRoleStats(digits);

    // CANONICAL ROLES — the same 1,000-tick digit-frequency state the ranking
    // uses. No digit is excluded from holding a role.
    ...
    return {
      ...
      total: slice.length,
      hot: state.green ?? -1,
      hot2: state.secondGreen ?? -1,
      cold: state.red ?? -1,
      cold2: state.secondRed ?? -1,
      rising: state.mostIncreasing ?? -1,
      falling: state.mostDecreasing ?? -1,
  ...
  return (
    <Panel title="Digits 0–9 Live Distribution" subtitle={`Last ${total} ticks · Expected 10% each`} accent="cyan">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] uppercase tracking-wider text-muted-foreground">
        ...
        <span>Falling <span className="font-semibold text-foreground">{falling >= 0 ? falling : "—"}</span></span>
